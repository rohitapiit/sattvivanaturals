...
export const sendReviewEmailManually = async (req, res) => {
  try {
    ...
    if (order.orderStatus !== "Delivered") {
      return res.status(400).json({
        success: false,
        message: "Review link can only be sent for delivered orders.",
      });
    }
    ...
    if (order.reviewEmailSentAt) {
      return res.status(400).json({
        success: false,
        message: "Review link has already been sent for this order.",
      });
    }

    await sendReviewEmail(order);

    // Check whether email was actually recorded
    const updatedOrder = await Order.findById(order._id);

    if (!updatedOrder.reviewEmailSentAt) {
      return res.status(500).json({
        success: false,
        message: "Review email could not be sent.",
      });
    }
    ...
    console.error("MANUAL REVIEW EMAIL ERROR:", error);
